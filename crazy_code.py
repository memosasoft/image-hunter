...
PATTERNS_IN_LINK = int(configuration.get('CONFIG', 'PATTERNS_IN_LINK'))
...
def save_global_memory_urls():
    ...
    learned_patterns = save_urls(learned_patterns, "./intelligence/learned_patterns")
...
def load_global_memory_url():
    ...
    learned_patterns = get_urls("./intelligence/learned_patterns", learned_patterns)
    ...
    for pattern in query_patterns[0].split(" "):
        ...
        if (link.find(stem) >= 0):
            ...
            if (len(pattern) > MIN_WORD_SIZE):
                ...
                if text_quality_score > PATTERNS_IN_LINK-1:
